...
* Remove all variables that contain the same val for each row, since they are not informative to distinguish two classes
...
importance(random.forest.mdl, scale = T) # reports the variables as more important if they are important for more common class
...
Let's select the ten most sifnificant variables obtained by Random Foresta alogorithm
...
for (j in 1:sampling.n.rep){
  ...
  eval.tbl.svm.final   <- cbind(Data=c("train", "test"), eval.tbl.svm)
  eval.tbl.svm.final
```
